...
export const MBCenterAllocationCounselling = () => {
        ...
        if (response.data.preference1 && Array.isArray(response.data.preference1)) {
          response.data.preference1.forEach(item => {
            if (item.center) {
              ...
              // Get counts from selected and waiting objects
              const selectedTotal = item.selected?.total || 0;
              const waitingTotal = item.waiting?.total || 0;
              
              counts[centerName].cp1Selected = selectedTotal;
              counts[centerName].cp1Waiting = waitingTotal;
              counts[centerName].cp1Total = selectedTotal + waitingTotal;
            }
          });
        }
        
        // Process preference2 data
        if (response.data.preference2 && Array.isArray(response.data.preference2)) {
          response.data.preference2.forEach(item => {
            if (item.center) {
              ...
              counts[centerName].cp2Selected = selectedTotal;
              counts[centerName].cp2Waiting = waitingTotal;
              counts[centerName].cp2Total = selectedTotal + waitingTotal;
            }
          });
        }
  ...
  const handleNextStudent = () => {
    ...
    // Refresh counts when moving to next student
    fetchPreferenceCounts();
  };
  ...
  const getCp1CountDisplay = (centerName) => {
    ...
    if (counts && counts.cp1Total > 0) {
      const selected = counts.cp1Selected || 0;
      const waiting = counts.cp1Waiting || 0;
  ...
  const getCp2CountDisplay = (centerName) => {
    ...
    if (counts && counts.cp2Total > 0) {
      const selected = counts.cp2Selected || 0;
      const waiting = counts.cp2Waiting || 0;
      return ` (${selected}/${waiting})`;
    }
    return "";
  };
                              ...
                              <td>
                                <div className="d-flex flex-column align-items-center gap-1">
                                  ...
                                  <Badge bg="primary" className="mt-1">
                                    S/W: {getCp1CountDisplay(center.centerName) || ' (0/0)'}
                                  </Badge>
                                </div>
                              </td>
                              <td>
                                <div className="d-flex flex-column align-items-center gap-1">
                                  ...
                                  <Badge bg="success" className="mt-1">
                                    S/W: {getCp2CountDisplay(center.centerName) || ' (0/0)'}
                                  </Badge>
                                </div>
                              </td>
                              <td>
