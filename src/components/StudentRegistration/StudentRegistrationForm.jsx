 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (studentData._id && loadedStudentIdRef.current !== studentData._id) {
      ...
      // Set context values from student data for update case
  ...
  // -----------------------
  // Clear local school state when context changes (FIX FOR THE BUG)
  // -----------------------
  useEffect(() => {
    // When district context is cleared, clear local school district state
    if (!districtContext) {
      setSchoolDistrict("");
  ...
  useEffect(() => {
    // When block context is cleared, clear local school block state
    if (!blockContext) {
      setSchoolBlock("");
      setSchoolBlockCode("");
    }
  ...
  useEffect(() => {
    // When school context is cleared, clear local school state
    if (!schoolContext) {
      setSchool("");
      setSchoolCode("");
    }
  ...
  const validateAll = () => {
    ...
    // CHANGED: Now require district, block, and school fields in BOTH create and update modes
...
const registrationDate =  new Date();
    ...
    try {
      ...
      // School details - FIXED: Always use context values when available, otherwise fallback to form state
      // This ensures that when user changes district/block/school in dropdowns, the updated values are used
