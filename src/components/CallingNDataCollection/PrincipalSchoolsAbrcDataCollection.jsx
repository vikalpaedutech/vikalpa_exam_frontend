...
//     abrcAssignedManualIds: "", // manual comma-separated IDs (for ABRC assigned schools)
//     abrcContact: "",
...
//     if (name === "principalContact" || name === "alternateSchoolNumber" || name === "abrcContact" || name === "abrcAlternateContact") {
...
//     // for ABRC manual assigned IDs, allow digits and commas only, sanitize spaces
//     if (name === "abrcAssignedManualIds") {
//       // keep digits, commas, spaces; normalize spaces and commas
//       const cleaned = value.replace(/[^\d,]/g, "").replace(/,+/g, ",").replace(/^\s*,|,\s*$/g, "");
//       setForm((f) => ({ ...f, [name]: cleaned }));
//       setErrors((e) => ({ ...e, [name]: null }));
//       return;
//     }
...
//       // ABRC assigned schools must have at least one id from dropdown or manual input
//       const selectedIds = (form.abrcAssignedSchools || []).map((o) => String(o.value));
//       const manualIdsStr = (form.abrcAssignedManualIds || "").trim();
//       const manualIds = manualIdsStr ? manualIdsStr.split(",").map(s => s.trim()).filter(Boolean) : [];
//       if (selectedIds.length === 0 && manualIds.length === 0) errs.abrcAssignedSchools = "Select at least one assigned school or add manual school IDs";
...
//         : (form.school ? form.school.value : "NA")
//       : "NA";

//     // ABRC assigned schools: combine selected ids + manual ids (both as strings), send only ids comma-separated
//     let abrcAssignedCsv = "NA";
//     if (form.dataType === "ABRC") {
//       const selectedIds = (form.abrcAssignedSchools || []).map((o) => String(o.value));
//       const manualRaw = (form.abrcAssignedManualIds || "").trim();
//       const manualList = manualRaw ? manualRaw.split(",").map(s => s.trim()).filter(Boolean) : [];
//       const combined = [...selectedIds, ...manualList].filter(Boolean);
...
//                     <Col md={6}>
//                       <Form.Group>
//                         <Form.Label>Data Filled By (auto)</Form.Label>
//                         <Form.Control type="text" readOnly value={form.dataFilledBy || ""} />
//                         {errors.dataFilledBy && <div className="text-danger small mt-1">{errors.dataFilledBy}</div>}
//                       </Form.Group>
//                     </Col>
...
//                               Selected dropdown schools will send their IDs. You can also add manual school IDs below (comma-separated).
//                             </Form.Text>
//                           </Form.Group>
//                         </Col>
...
//                         <Col md={12}>
//                           <Form.Group>
//                             <Form.Label>Manual Assigned School IDs (optional)</Form.Label>
//                             <Form.Control
//                               type="text"
//                               name="abrcAssignedManualIds"
//                               value={form.abrcAssignedManualIds}
//                               onChange={handleInputChange}
//                               placeholder="Enter school IDs comma-separated (e.g. 104,108,146)"
//                             />
//                             <Form.Text className="text-muted d-block">
//                               If some schools are not in dropdown, enter their IDs here. Both dropdown IDs and manual IDs will be merged and sent as comma-separated IDs.
//                             </Form.Text>
//                           </Form.Group>
//                         </Col>
//                       </>
...
//                   Tip: For ABRC assigned schools, selected dropdown schools send IDs. Manual school IDs can be added and will be merged with selected IDs before saving. Alternate contact fields are optional.
//                 </small>
//               </Card.Footer>
//             </Card>
//           </Col>
