...
//Level 2 Admit card download mb
...
//       // Use admitCardCount8 for class 8 and admitCardCount10 for class 10
//       const admitCardDownloadCount = classType === '8'
//         ? Number(school?.admitCardCount8 || 0)
//         : Number(school?.admitCardCount10 || 0);
...
//       districtsMap[districtId].totalRegistered += registrationCount;
//       districtsMap[districtId].totalAdmitCardDownloads += admitCardDownloadCount;
//       districtsMap[districtId].totalSchools += 1;
...
//           blockName,
//           registered: 0,
//           admitCardDownloads: 0,
//           lowRegistrationSchools: []
//         };
//       }

//       districtsMap[districtId].blocks[blockId].registered += registrationCount;
//       districtsMap[districtId].blocks[blockId].admitCardDownloads += admitCardDownloadCount;
...
//           schoolName,
//           registrationCount,
//           admitCardDownloads: admitCardDownloadCount
//         });
//         districtsMap[districtId].totalLowRegistrationSchools += 1;
//       }
...
//       blocksMap[blockId].totalRegistered += registrationCount;
//       blocksMap[blockId].totalAdmitCardDownloads += admitCardDownloadCount;
...
//       blocksMap[blockId].schools[schoolId].registered += registrationCount;
//       blocksMap[blockId].schools[schoolId].admitCardDownloads += admitCardDownloadCount;
//     });
...
//     // Total Registration and Admit Card Downloads
//     pdf.setFontSize(12);
//     pdf.setFont("helvetica", "normal");
//     pdf.text(`Total Registrations (Class ${classType}): ${district.totalRegistered}`, pageWidth / 2, 30, { align: "center" });
//     pdf.text(`Total Admit Card Downloads (Class ${classType}): ${district.totalAdmitCardDownloads}`, pageWidth / 2, 37, { align: "center" });
    
//     // Date below Total Registration
//     const currentDate = new Date().toLocaleDateString();
//     pdf.setFontSize(10);
//     pdf.setFont("helvetica", "italic");
//     pdf.text(`Report Date: ${currentDate}`, pageWidth / 2, 44, { align: "center" });
    
//     // Header for blocks table
//     pdf.autoTable({
//       startY: 50,
//       head: [['#', 'Block Name', 'Registration Count', 'Admit Card Downloads']],
//       body: district.blocks.map((block, index) => [
//         index + 1,
//         block.blockName,
//         block.registered.toString(),
//         block.admitCardDownloads.toString()
//       ]),
//       theme: 'grid',
//       headStyles: { fillColor: [66, 139, 202] },
//       styles: { fontSize: 10 },
//       columnStyles: {
//         0: { cellWidth: 10 }, // #
//         1: { cellWidth: 60 }, // Block Name
//         2: { cellWidth: 30 }, // Registration Count
//         3: { cellWidth: 30 }  // Admit Card Downloads
//       }
//     });
...
//           school.schoolName,
//           school.registrationCount.toString(),
//           school.admitCardDownloads.toString()
//         ]);

//         pdf.autoTable({
//           startY: finalY,
//           head: [['#', 'School Name', 'Count', 'Admit Card Downloads']],
//           body: lowSchoolsData,
//           theme: 'grid',
//           headStyles: { fillColor: [220, 53, 69] },
//           styles: { fontSize: 8 },
...
//           columnStyles: {
//             0: { cellWidth: 10 }, // #
//             1: { cellWidth: 80 }, // School Name
//             2: { cellWidth: 20 }, // Count
//             3: { cellWidth: 30 }  // Admit Card Downloads
//           }
//         });
...
//     pdf.setFont("helvetica", "normal");
//     pdf.text(`Total Registrations (Class ${classType}): ${block.totalRegistered}`, pageWidth / 2, 30, { align: "center" });
//     pdf.text(`Total Admit Card Downloads (Class ${classType}): ${block.totalAdmitCardDownloads}`, pageWidth / 2, 37, { align: "center" });
    
//     // Date below Total Registration
//     const currentDate = new Date().toLocaleDateString();
//     pdf.setFontSize(10);
//     pdf.setFont("helvetica", "italic");
//     pdf.text(`Date: ${currentDate}`, pageWidth / 2, 44, { align: "center" });
    
//     // Schools table
//     pdf.autoTable({
//       startY: 50,
//       head: [['#', 'School Name', 'Registration Count', 'Admit Card Downloads']],
//       body: block.schools.map((school, index) => [
//         index + 1,
//         school.schoolName,
//         school.registered.toString(),
...
//                     <h6>District-Block Reports</h6>
//                     <p className="small text-muted">
//                       Individual PDFs for each district showing block-wise registration counts, admit card downloads, and schools with ≤5 registrations.
//                     </p>
//                   </div>
//                   <Button 
//                     variant="outline-primary" 
...
//                     <h6>Block-School Reports</h6>
//                     <p className="small text-muted">
//                       Individual PDFs for each block showing school-wise registration counts and admit card downloads.
//                     </p>
//                   </div>
//                   <Button 
//                     variant="outline-primary" 
...
//               <h6>Report Format Details:</h6>
//               <ul className="mb-0 small">
//                 <li><strong>District-Block Reports:</strong> Each PDF contains district summary, block-wise counts, admit card downloads, and list of schools with ≤5 registrations</li>
//                 <li><strong>Block-School Reports:</strong> Each PDF contains block summary and school-wise registration counts and admit card downloads</li>
//                 <li>All reports include admit card download counts for both class 8 and class 10</li>
//                 <li>All reports are sorted by registration count (descending)</li>
//                 <li>Files are organized in ZIP format for easy download</li>
//               </ul>
//             </Card.Body>
...
export const DownloadDashboardReports = () => {
  ...
  const processDashboardData = (mainData, classType) => {
    ...
    mainData.forEach(school => {
      ...
      // Process blocks independently for block-school reports
      const blockKey = `${districtId}_${blockId}`;
      ...
      blocksMap[blockKey].totalRegistered += registrationCount;
      blocksMap[blockKey].totalAdmitCardDownloads += admitCardDownloadCount;

      if (!blocksMap[blockKey].schools[schoolId]) {
        blocksMap[blockKey].schools[schoolId] = {
          schoolId,
          schoolName,
          registered: 0,
          admitCardDownloads: 0
      ...
      blocksMap[blockKey].schools[schoolId].registered += registrationCount;
      blocksMap[blockKey].schools[schoolId].admitCardDownloads += admitCardDownloadCount;
    });
    ...
    // Group blocks by district for block-school reports
    ...
    blocks.forEach(block => {
      ...
      blocksByDistrict[block.districtId].blocks.push(block);
    });

    return { districts, blocks, blocksByDistrict };
  };
  ...
  const generateBlockSchoolPDF = (block, classType) => {
    ...
    pdf.setFontSize(12);
    pdf.setFont("helvetica", "normal");
    pdf.text(`District: ${block.districtName}`, pageWidth / 2, 30, { align: "center" });
    
    // Total Registration and Admit Card Downloads
    pdf.text(`Total Registrations (Class ${classType}): ${block.totalRegistered}`, pageWidth / 2, 37, { align: "center" });
    pdf.text(`Total Admit Card Downloads (Class ${classType}): ${block.totalAdmitCardDownloads}`, pageWidth / 2, 44, { align: "center" });
    
    // Date below Total Registration
    const currentDate = new Date().toLocaleDateString();
    pdf.setFontSize(10);
    pdf.setFont("helvetica", "italic");
    pdf.text(`Date: ${currentDate}`, pageWidth / 2, 51, { align: "center" });
  ...
  const downloadBlockSchoolReports = async (classType) => {
    ...
    try {
      ...
      const { blocksByDistrict } = processDashboardData(mainData, classType);
      
      if (Object.keys(blocksByDistrict).length === 0) {
        setError("No block data available for download.");
        return;
      }

      const zip = new JSZip();
      const mainFolder = zip.folder(`class-${classType}-block-school-reports`);

      let totalBlocks = 0;

      // Create district-wise folders and add block PDFs
      Object.values(blocksByDistrict).forEach(district => {
        // Create folder for each district
        const districtFolder = mainFolder.folder(district.districtName.replace(/[^a-zA-Z0-9]/g, '_'));
        
        // Add block PDFs to district folder
        district.blocks.forEach(block => {
          const pdf = generateBlockSchoolPDF(block, classType);
          const fileName = `${block.blockName.replace(/[^a-zA-Z0-9]/g, '_')}_class_${classType}.pdf`;
          districtFolder.file(fileName, pdf.output('blob'));
          totalBlocks++;
        });
      });
      ...
      setSuccess(`Successfully downloaded ${totalBlocks} block reports across ${Object.keys(blocksByDistrict).length} districts for Class ${classType}`);
              ...
              <Card className="h-100 border-primary">
                ...
                <Card.Body className="d-flex flex-column">
                  ...
                  <div className="mb-3">
                    <h6>Block-School Reports</h6>
                    <p className="small text-muted">
                      Individual PDFs for each block showing school-wise registration counts and admit card downloads. Organized in district-wise folders.
                    </p>
                  </div>
                  <Button 
                    variant="outline-primary" 
          ...
          <Card className="mt-4 bg-light">
            <Card.Body>
              ...
              <ul className="mb-0 small">
                <li><strong>District-Block Reports:</strong> Each PDF contains district summary, block-wise counts, admit card downloads, and list of schools with ≤5 registrations</li>
                <li><strong>Block-School Reports:</strong> Each PDF contains block summary and school-wise registration counts and admit card downloads</li>
                <li><strong>Block-School ZIP Structure:</strong> Block reports are organized in district-wise folders within the ZIP file</li>
                <li>All reports include admit card download counts for both class 8 and class 10</li>
                <li>All reports are sorted by registration count (descending)</li>
                <li>Files are organized in ZIP format for easy download</li>
              </ul>
