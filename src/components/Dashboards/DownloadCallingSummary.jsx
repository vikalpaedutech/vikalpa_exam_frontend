...
//       doc.text(`Report Date: ${summaryData.dateRange.startDate}`, 14, 25);
//     } else {
//       doc.text(`Report Date: ${summaryData.dateRange.startDate} to ${summaryData.dateRange.endDate}`, 14, 25);
//     }
...
export const GetCallSummary = () => {
  ...
  const downloadPDF = () => {
    ...
    if (formData.callMadeTo === "BEO") {
      ...
      // Sort summaryByDistrictBlock by district name
      const sortedDistrictBlocks = [...summaryData.summaryByDistrictBlock].sort((a, b) => {
        const districtA = a.districtName || a.districtId || "";
        const districtB = b.districtName || b.districtId || "";
        return districtA.localeCompare(districtB);
      });
      
      sortedDistrictBlocks.forEach(districtBlock => {
        const districtName = districtBlock.districtName || districtBlock.districtId;
      ...
      // Convert to table rows and sort by district name
      const sortedCallerData = Object.values(districtCallerMap).sort((a, b) => {
        return a.districtName.localeCompare(b.districtName);
      });
      
      sortedCallerData.forEach(callerData => {
        tableRows.push([
          serialNumber.toString(),
          callerData.callerName,
          callerData.districtName,
      ...
      // For Principal, ABRC, DEO - keep original logic but sort by district and block
      // Sort summaryByDistrictBlock by district name and then by block name
      const sortedDistrictBlocks = [...summaryData.summaryByDistrictBlock].sort((a, b) => {
        const districtA = a.districtName || a.districtId || "";
        const districtB = b.districtName || b.districtId || "";
        const blockA = a.blockName || a.blockId || "";
        const blockB = b.blockName || b.blockId || "";
  ...
  const downloadExcel = () => {
    ...
    if (formData.callMadeTo === "BEO") {
      ...
      // Convert to excel rows and sort by district name
      const sortedCallerData = Object.values(districtCallerMap).sort((a, b) => {
        return a.districtName.localeCompare(b.districtName);
      });
              ...
              <tbody>
                {formData.callMadeTo === "BEO" ? (
                  ...
                  (() => {
                    ...
                    // Sort caller data by district name
                    const sortedCallerData = Object.values(districtCallerMap).sort((a, b) => {
                      return a.districtName.localeCompare(b.districtName);
                    });
                    
                    return sortedCallerData.map((callerData, index) => (
                      <tr 
                        key={`${callerData.districtName}_${callerData.callerName}`}
                        style={{ 
                          backgroundColor: index % 2 === 0 ? "#f8f9fa" : "white",
                  ...
                  (() => {
                    ...
                    return sortedDistrictBlocks.map((districtBlock, index) => (
                      districtBlock.assignedCallers.map((caller, callerIndex) => (
                        <tr 
                          key={`${index}-${callerIndex}`}
                          style={{ 
