...
export const DeoCallings = () => {
  ...
  // Function to calculate total district sums for Class 8 and Class 10
  const getDistrictTotalSnippets = (schools) => {
    let totalClass8 = 0;
    let totalClass10 = 0;
  ...
  const fetchCallLeads = async () => {
    ...
    setError(null);
    try {
      const reqBody = { objectIdOfCaller: userData?.user?._id, callMadeTo: "DEO" };
...
console.log(data)
        ...
        groupsArr.push({
          ...
          current: {
            ...
            calledPerson: current.calledPerson || {
              ...
              deo: current.deo || "N/A",
              deoContact: current.deoContact || null,
  ...
  // Function to aggregate school data by district only
  ...
  return (
    <Container className="py-3">
      <h3 className="mb-3">DEO Callings</h3>
      ...
      <Row xs={1} md={2} lg={2} className="g-3">
        {groups.map(g => {
          ...
          // Use getDistrictTotalSnippets for DEO
          const snippets = getDistrictTotalSnippets(lead.calledPersonRegion || []);
          ...
          // Get aggregated school data for this DEO
            ...
            <Col key={g.key}>
              <Card className={submitted ? "border-success" : ""} style={submitted ? { backgroundColor: "#d4edda" } : {}}>
                <Card.Header className="d-flex justify-content-between align-items-start">
                  <div>
                    <div style={{ fontSize: "1.5rem", fontWeight: "bold" }}>{lead.calledPerson?.deo}</div>
                    <div className="small text-muted">{lead.calledPerson?.districtName}</div>
                    {snippets.length > 0 && <div className="small text-muted mt-1">{snippets.join(" · ")}</div>}
                    {lead.calledPerson?.deoContact
                      ? <div className="small mt-1">Contact: <a href={`tel:${lead.calledPerson.deoContact}`}>{lead.calledPerson.deoContact}</a></div>
                      : <div className="small mt-1">Contact: N/A</div>}
                  </div>
                  <div className="text-end">
                    <Badge bg="primary" className="mb-1">{lead.callMadeTo}</Badge>
                    <div className="small text-muted">District-Id: {lead.districtId}</div>
                ...
                <Card.Body style={{ padding: "0.5rem" }}>
                  ...
                  {/* DEO Schools Registration Table - Aggregated by District Only */}
                  {aggregatedData.length > 0 && (
                    <Card className="mb-2">
                      <Card.Header><strong>DEO Schools Registration Summary</strong></Card.Header>
