...
//   // State for call logs fetched (today + previous 5 days) and mapping per ABRC
//   const [callLogsRange, setCallLogsRange] = useState([]); // raw logs array
//   // structure: { "<abrc>||<contact>": { calledToday: bool, todayLog: {}, previousLogs: [] } }
//   const [abrcCallStatusMap, setAbrcCallStatusMap] = useState({}); // { "<abrc>||<contact>": { calledToday: bool, previousRemarks: [] } }
//   const [loadingCallLogs, setLoadingCallLogs] = useState(false);

//   // Inline UI states (no alerts)
//   const [formErrors, setFormErrors] = useState({}); // { simpleKey: 'error text' }
//   const [successMessages, setSuccessMessages] = useState({}); // { simpleKey: 'Success text' }
...
//       }
//     }));
//     // clear previous errors for this group when user changes input
//     setFormErrors(prev => ({ ...prev, [abrcKey]: undefined }));
//   };
...
//         const isToday = logIstDate === todayIstString;
//         if (!map[key]) {
//           map[key] = { calledToday: false, todayLog: null, previousLogs: [] };
//         }
//         if (isToday) {
//           map[key].calledToday = true;
//           // keep the latest today log (if multiple, keep last encountered)
//           map[key].todayLog = log;
//         } else {
//           // collect previous (not-today) full log objects
//           map[key].previousLogs.push({
//             createdAt: log.createdAt,
//             callingStatus: log.callingStatus ?? "",
//             callingRemark: log.callingRemark ?? "",
//             manualRemark: log.manualRemark ?? ""
//           });
//         }
//       }

//       // sort previousLogs by date desc and dedupe identical entries (by JSON)
//       Object.keys(map).forEach(k => {
//         const arr = map[k].previousLogs || [];
//         // sort newest first
//         arr.sort((a, b) => {
...
//             callingStatus: (item.callingStatus || "").toString().trim(),
//             callingRemark: (item.callingRemark || "").toString().trim(),
//             manualRemark: (item.manualRemark || "").toString().trim(),
//             createdAt: toIstDateString(item.createdAt)
//           });
//           if (!seen.has(keyStr)) {
//             seen.add(keyStr);
//             deduped.push(item);
...
//  // sort abrcGroups so that uncalled ones appear first (calledToday groups go to bottom)
//  const sortedAbrcGroups = useMemo(() => {
//    if (!Array.isArray(abrcGroups)) return [];
//    // Use abrc||contact as key for lookup
//    return [...abrcGroups].sort((a, b) => {
//      const aKey = `${a.abrc || ""}||${a.abrcContact || ""}`;
//      const bKey = `${b.abrc || ""}||${b.abrcContact || ""}`;
//      const aCalled = abrcCallStatusMap[aKey]?.calledToday ? 1 : 0;
//      const bCalled = abrcCallStatusMap[bKey]?.calledToday ? 1 : 0;
//      // want uncalled (0) before called (1)
//      if (aCalled === bCalled) {
//        // keep alphabetical if both same called state
//        return (a.abrc || "").localeCompare(b.abrc || "");
//      }
//      return aCalled - bCalled;
//    });
//  }, [abrcGroups, abrcCallStatusMap]);

//  // prepare defaultActiveKey array so all accordions open by default (multi-open)
//  const defaultActiveKeys = useMemo(() => {
//    return sortedAbrcGroups.map((_, i) => String(i));
//  }, [sortedAbrcGroups]);

//  // replace the current handleSubmit with this function in your component
//  const handleSubmit = async (simpleKey, group) => {
//   try {
//     const data = formData[simpleKey] || {};

//     // Basic validation (no browser alerts; show inline errors)
//     if (!data.callingStatus) {
//       setFormErrors(prev => ({ ...prev, [simpleKey]: 'Please select calling status' }));
//       return;
//     }
//     if (!data.callingRemark) {
//       setFormErrors(prev => ({ ...prev, [simpleKey]: 'Please select calling remark' }));
//       return;
//     }
...
//     const callerId = userData?.user?._id ?? userData?._id ?? null;
//     if (!callerId) {
//       setFormErrors(prev => ({ ...prev, [simpleKey]: 'User information not found. Please login again.' }));
//       return;
//     }
...
//     if (res && (res.success || res.data)) {
//       // clear form
//       setFormData(prev => ({ ...prev, [simpleKey]: undefined }));
//       // set a success message for this group (inline)
//       setSuccessMessages(prev => ({ ...prev, [simpleKey]: 'Call log created successfully' }));
//       // refresh logs so UI disables group after successful call
//       await fetchCallLogs();
//       // clear success message after short delay (optional)
//       setTimeout(() => {
//         setSuccessMessages(prev => ({ ...prev, [simpleKey]: undefined }));
//       }, 3500);
//     } else {
//       console.error("CreateCallLogs response:", res);
//       setFormErrors(prev => ({ ...prev, [simpleKey]: res?.message || "Failed to create call log" }));
//     }
//   } catch (error) {
//     console.error("Error creating call log:", error);
//     setFormErrors(prev => ({ ...prev, [simpleKey]: "Error creating call log. Please try again." }));
//   }
// };
...
//               {sortedAbrcGroups.map((group, idx) => {
//                 const headerLabel = group.abrc || "Unknown ABRC";
//                 const contact = group.abrcContact || "";
//                 const simpleKey = `${headerLabel}||${contact}`; // used to lookup call status map and form state
//                 const abrcKey = simpleKey; // use simpleKey as the stable key for formData/errors/success
//                 const currentFormData = formData[abrcKey] || {};
//                 const statusEntry = abrcCallStatusMap[simpleKey] || { calledToday: false, todayLog: null, previousLogs: [] };
//                 const disabled = !!statusEntry.calledToday;

//                 // header background: until the call is made (i.e., not calledToday) -> green; otherwise red
//                 const headerBg = statusEntry.calledToday ? '#d4edda' : '#ffffff';
//                 const headerColor = statusEntry.calledToday ? '#0f5132' : '#212529';
//                 const headerBorder = statusEntry.calledToday ? '1px solid #c3e6cb' : '1px solid #e9ecef';

//                 const callingStatusText = statusEntry.todayLog?.callingStatus ?? (statusEntry.calledToday ? 'Called' : 'Not called');

//                 // Build displayLogs which includes today's log (if present) followed by previous logs
//                 const displayLogs = [];
//                 if (statusEntry.todayLog) {
//                   displayLogs.push({
//                     createdAt: statusEntry.todayLog.createdAt,
//                     callingStatus: statusEntry.todayLog.callingStatus ?? "",
//                     callingRemark: statusEntry.todayLog.callingRemark ?? "",
//                     manualRemark: statusEntry.todayLog.manualRemark ?? ""
//                   });
//                 }
//                 if (statusEntry.previousLogs && statusEntry.previousLogs.length) {
//                   displayLogs.push(...statusEntry.previousLogs);
//                 }

//                 return (
//                   <Accordion.Item eventKey={String(idx)} key={abrcKey}>
//                     <Accordion.Header>
//                       <div style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: '6px 8px', borderRadius: 4, background: headerBg, color: headerColor, border: headerBorder }}>
//                         <div>
//                           <strong style={{ display: 'block' }}>{headerLabel}</strong>
//                           <div style={{ fontSize: "0.85rem", color: headerColor }}>
//                             {group.schools.length} school{group.schools.length > 1 ? "s" : ""}
//                           </div>
//                         </div>
//                         <div style={{ textAlign: "right", minWidth: 220 }}>
//                           <div style={{ fontSize: '0.95rem', fontWeight: 600 }}>{contact ? <a href={`tel:${contact}`} style={{ color: headerColor, textDecoration: 'none' }}>{contact}</a> : <span style={{ color: '#6c757d' }}>No contact</span>}</div>
//                           <div style={{ fontSize: '0.85rem', marginTop: 3 }}>Calling Status: <span style={{ fontWeight: 600 }}>{callingStatusText}</span></div>
//                         </div>
//                       </div>
//                     </Accordion.Header>
...
//                       {/* Previous remarks (from earlier days in fetched range) - now include today's log at top */}
//                       {displayLogs && displayLogs.length > 0 && (
//                         <Card className="mb-3">
//                           <Card.Header><strong>Last calling remarks</strong></Card.Header>
//                           <Card.Body>
//                             <Table striped bordered hover responsive size="sm" className="mb-0">
//                               <thead>
//                                 <tr>
//                                   <th style={{ width: 120 }}>Calling Date (IST)</th>
//                                   <th style={{ width: 140 }}>Calling Status</th>
...
//                                 {displayLogs.map((r, i) => (
//                                   <tr key={i}>
//                                     <td>{toIstDateString(r.createdAt) || '-'}</td>
//                                     <td>{r.callingStatus || '-'}</td>
//                                     <td>{r.callingRemark || '-'}</td>
//                                     <td>{r.manualRemark || '-'}</td>
//                                   </tr>
//                                 ))}
//                               </tbody>
//                             </Table>
...
//                             <Row className="mb-2">
//                               <Col>
//                                 <div style={{ color: '#842029', fontWeight: 600 }}>{formErrors[abrcKey]}</div>
//                               </Col>
//                             </Row>
//                           )}

//                           {/* inline success message */}
//                           {successMessages[abrcKey] && (
//                             <Row className="mb-2">
//                               <Col>
//                                 <Alert variant="success" className="py-1 my-0">
//                                   {successMessages[abrcKey]}
//                                 </Alert>
//                               </Col>
//                             </Row>
...
//     // Build a quick lookup map by centerId for faster access in UI
...
//   // dashboard lookup by centerId
//   const [dashboardMap, setDashboardMap] = useState({}); // { "<centerId>": dashboardCounts }
...
//   <tbody>
//     {group.schools.map((s, i) => {
//       const countsObj = dashboardMap?.[String(s.centerId)] || {};
//       const schoolCounts = countsObj.school || {};
//       const byClass = schoolCounts.byClass || {};
//       const class8 = byClass["8"] || {};
//       const class10 = byClass["10"] || {};

//       const class8Reg = class8.registered ?? 0;
//       const class8Ver = class8.verified ?? 0;
//       const class10Reg = class10.registered ?? 0;
//       const class10Ver = class10.verified ?? 0;
          ...
          <Col>
            <Accordion alwaysOpen defaultActiveKey={defaultActiveKeys}>
              {sortedAbrcGroups.map((group, idx) => {
                ...
                // compute total registrations for this ABRC group (class 8 & class 10)
                let totalClass8Reg = 0;
                let totalClass10Reg = 0;
                group.schools.forEach((s) => {
                  const countsObj = dashboardMap?.[String(s.centerId)] || {};
                  const schoolCounts = countsObj.school || {};
                  const byClass = schoolCounts.byClass || {};
                  const c8 = byClass["8"] || {};
                  const c10 = byClass["10"] || {};
                  totalClass8Reg += c8.registered ?? 0;
                  totalClass10Reg += c10.registered ?? 0;
                });
                    ...
                    <Accordion.Header>
                      <div style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: '6px 8px', borderRadius: 4, background: headerBg, color: headerColor, border: headerBorder }}>
                        <div>
                          <strong style={{ display: 'block' }}>{headerLabel}</strong>
                          <div style={{ fontSize: "0.85rem", color: headerColor }}>
                            Total Schools: {group.schools.length} {group.schools.length > 1 ? "s" : ""}
                          </div>
                          <hr></hr>
                          ...
                          <div style={{ fontSize: '0.85rem', marginTop: 4 }}>
                            <span style={{ fontWeight: 600 }}>Class 8 Reg: </span>{totalClass8Reg} &nbsp; <span style={{ fontWeight: 600 }}>Class 10 Reg: </span>{totalClass10Reg}
                          </div>
                           <hr></hr>
                          <div style={{ fontSize: '0.95rem', fontWeight: 600 }}>{contact ? <a href={`tel:${contact}`} style={{ color: headerColor, textDecoration: 'none' }}>{contact}</a> : <span style={{ color: '#6c757d' }}>No contact</span>}</div>
                           <hr></hr>
