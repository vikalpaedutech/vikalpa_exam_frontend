...
//   const [editingStatus, setEditingStatus] = useState("");
//   const [editingRemark, setEditingRemark] = useState("");
//   const [editingManualRemark, setEditingManualRemark] = useState(""); // NEW: state for manual remark
...
//     { value: "School denied for exam", label: "School denied for exam" },
//     { value: "Not interested", label: "Not interested" },
//     { value: "Travel issue due to long distance", label: "Travel issue due to long distance" },
//     { value: "Could not reach exam center due to long distance", label: "Could not reach exam center due to long distance" },
//     // { value: "MB Level 1 exam not given", label: "MB Level 1 exam not given" },
//   ];
...
//     setEditingStatus(student.callingStatus || "");
//     setEditingRemark(student.remark || "");
//     setEditingManualRemark(student.manualRemark || ""); // NEW: initialize manual remark
//   };
...
//     setEditingStatus("");
//     setEditingRemark("");
//     setEditingManualRemark(""); // NEW: clear manual remark
//   };
...
//         callingStatus: editingStatus,
//         remark: editingRemark,
//         manualRemark: editingManualRemark // NEW: include manual remark in request
//       };
...
//                   callingStatus: editingStatus, 
//                   remark: editingRemark,
//                   manualRemark: editingManualRemark // NEW: update manual remark
//                 } 
//               : student
//           );
...
//                       <strong>Remark:</strong> {student.remark || "No remark"}
//                     </p>
//                     {/* NEW: Display manual remark if exists */}
//                     {student.manualRemark && (
//                       <p className="mb-1">
//                         <strong>Manual Remark:</strong> {student.manualRemark}
//                       </p>
//                     )}
//                   </div>
                  
//                   {/* Edit Form - Always shown if editing this student */}
...
//                       {/* NEW: Manual remark text input */}
//                       <Form.Group className="mb-3">
//                         <Form.Label>Manual Remark (Optional)</Form.Label>
//                         <Form.Control
//                           as="textarea"
//                           rows={2}
//                           placeholder="Enter any additional remarks here..."
//                           value={editingManualRemark}
//                           onChange={(e) => setEditingManualRemark(e.target.value)}
//                         />
//                         <Form.Text className="text-muted">
//                           Additional custom notes about the call
//                         </Form.Text>
//                       </Form.Group>
...
//                   ) : (
//                     <Button
//                       variant={hasBeenCalled(student) ? "outline-success" : "primary"}
//                       size="sm"
//                       onClick={() => startEditing(student)}
//                     >
//                       {hasBeenCalled(student) ? "Update Call Record" : "Update Calling Status"}
//                     </Button>
//                   )}
//                 </Card.Body>
...
export const CallingsMB2 = () => {
  ...
  const [editingStatus, setEditingStatus] = useState("");
  const [editingRemark, setEditingRemark] = useState("");
  const [editingManualRemark, setEditingManualRemark] = useState("");
  // NEW: state for distance input
  const [editingDistance, setEditingDistance] = useState("");
  // NEW: state to track if distance field should be shown
  ...
  // NEW: Effect to check if selected remark requires distance field
  useEffect(() => {
    const distanceRemarkOptions = [
      "Travel issue due to long distance",
      "Could not reach exam center due to long distance"
    ];
    
    if (distanceRemarkOptions.includes(editingRemark)) {
      setShowDistanceField(true);
    } else {
      setShowDistanceField(false);
      setEditingDistance(""); // Clear distance when not needed
    }
  }, [editingRemark]);
  ...
  const startEditing = (student) => {
    ...
    // Parse the remark and distance if they exist
    if (student.remark && student.remark.includes("||")) {
      const [remark, distance] = student.remark.split("||");
      setEditingRemark(remark || "");
      setEditingDistance(distance || "");
      setEditingManualRemark(""); // Reset manual remark when editing
    } else {
      setEditingRemark(student.remark || "");
      setEditingDistance("");
      setEditingManualRemark(student.manualRemark || "");
    }
    
    // Check if the remark requires distance field
    const distanceRemarkOptions = [
      "Travel issue due to long distance",
      "Could not reach exam center due to long distance"
    ];
    setShowDistanceField(distanceRemarkOptions.includes(student.remark || ""));
  };
  ...
  const handleUpdateCallingStatus = async () => {
    ...
    // NEW: Validate distance if required
    if (showDistanceField && !editingDistance) {
      setError("Please enter the distance in kilometers");
      return;
    }
    
    // NEW: Validate distance is a valid number
    if (showDistanceField && editingDistance && isNaN(parseFloat(editingDistance))) {
      setError("Please enter a valid number for distance");
      return;
    }
    ...
    try {
      // NEW: Construct final remark with distance if applicable
      let finalRemark = editingRemark;
      if (showDistanceField && editingDistance) {
        finalRemark = `${editingRemark}||${editingDistance}`;
      }
      
      const reqBody = {
        _id: editingId,
        callingStatus: editingStatus,
        remark: finalRemark, // Use the constructed remark with distance
        manualRemark: editingManualRemark
      };
          ...
          const updatedStudents = prevStudents.map(student => 
            student._id === editingId 
              ? { 
                  ...student, 
                  callingStatus: editingStatus, 
                  remark: finalRemark, // Save the constructed remark
                  manualRemark: editingManualRemark
                } 
              : student
          );
  ...
  // Helper function to extract distance from remark for display
  const getDisplayRemark = (remark) => {
    if (!remark) return "No remark";
    if (remark.includes("||")) {
      const [remarkText, distance] = remark.split("||");
      return (
        <>
          {remarkText}
          {distance && (
              ...
              <Card className={`h-100 shadow-sm ${getCardBorderColor(student)}`}>
                ...
                <Card.Body>
                  ...
                  <div className="mb-3">
                    ...
                    <p className="mb-1">
                      <strong>Remark:</strong> {getDisplayRemark(student.remark)}
                    </p>
                    {student.manualRemark && (
                      <p className="mb-1">
                        <strong>Manual Remark:</strong> {student.manualRemark}
                  ...
                  {editingId === student._id ? (
                    <div className="border rounded p-3 bg-light">
                      ...
                      {/* NEW: Distance input field - shown only when distance-related remark is selected */}
                      {showDistanceField && (
                        <Form.Group className="mb-3">
                          <Form.Label>
                            Distance from home to exam center (km) <span className="text-danger">*</span>
                          </Form.Label>
                          <Form.Control
                            type="number"
                            step="any"
                          ...
                          />
                          <Form.Text className="text-muted">
                            Please enter the distance in kilometers (e.g., 5.5, 10, 15.75)
                          </Form.Text>
                        </Form.Group>
                      )}
                      ...
                      <div className="d-flex gap-2">
                        <Button
                          variant="primary"
                          onClick={handleUpdateCallingStatus}
                          disabled={updating || !editingStatus || (showDistanceField && !editingDistance)}
                        >
                          {updating ? (
                            <Spinner animation="border" size="sm" />
                          ) : (
